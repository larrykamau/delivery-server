...
class StaffDeleteMixin(UserDeleteMixin):
    ...
    def check_if_users_can_be_deleted(cls, info, instances, field, errors):
        ...
        if not_staff_users:
            user_pks = [
                graphene.Node.to_global_id("CustomUser", user.pk) for user in not_staff_users
            ]
    ...
    def check_if_requestor_can_manage_users(cls, requestor, instances, field, errors):
        ...
        if out_of_scope_users:
            user_pks = [
                graphene.Node.to_global_id("CustomUser", user.pk)
...
def get_user_permissions(user: "CustomUser") -> "QuerySet":
...
def get_out_of_scope_permissions(
    requestor: Union["CustomUser",], permissions: List[str]
) -> List[str]:
...
def get_out_of_scope_users(root_user: "CustomUser", users: List["CustomUser"]):
...
def can_user_manage_group(user: "CustomUser", group: Group) -> bool:
...
# def can_manage_app(requestor: Union["CustomUser", "App"], app: "App") -> bool:
...
def get_groups_which_user_can_manage(user: "CustomUser") -> List[Optional[Group]]:
...
def get_not_manageable_permissions_when_deactivate_or_remove_users(users: List["CustomUser"]):
...
def get_not_manageable_permissions_after_removing_users_from_group(
    group: Group, users: List["CustomUser"]
):
