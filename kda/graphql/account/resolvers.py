...
def resolve_customers(info, query, **_kwargs):
    qs = models.CustomUser.objects.customers()
...
def resolve_staff_users(info, query, **_kwargs):
    qs = models.CustomUser.objects.staff()
    qs = filter_by_query_param(
        queryset=qs, query=query, search_fields=USER_SEARCH_FIELDS
    )
    return qs.distinct()
...
def resolve_user(info, id):
    ...
    if requester:
        ...
        if requester.has_perms(
            ...
            return models.CustomUser.objects.filter(pk=user_pk).first()
        if requester.has_perm(AccountPermissions.MANAGE_STAFF):
            return models.CustomUser.objects.staff().filter(pk=user_pk).first()
        if requester.has_perm(AccountPermissions.MANAGE_USERS):
            return models.CustomUser.objects.customers().filter(pk=user_pk).first()
...
# def resolve_payment_sources(user: models.CustomUser):
...
def resolve_permissions(root: models.CustomUser):
