...
class CustomerEvent(CountableDjangoObjectType):
    ...
    order = graphene.Field(
        "kda.graphql.order.types.Order", description="The concerned order."
    )
    order_line = graphene.Field(
        "kda.graphql.order.types.OrderLine", description="The concerned order line."
    )
...
class UserPermission(Permission):
    source_permission_groups = graphene.List(
        graphene.NonNull("kda.graphql.account.types.Group"),
...
class User(CountableDjangoObjectType):
    ...
    #     "kda.graphql.giftcard.types.GiftCard",
    ...
    orders = PrefetchingConnectionField(
        "kda.graphql.order.types.Order", description="List of user's orders."
    )
    ...
    permissions = graphene.List(
        ...
        deprecation_reason=(
            "Will be removed in kda 2.11." "Use the `userPermissions` instead."
        ),
    )
    ...
    permission_groups = graphene.List(
        "kda.graphql.account.types.Group",
    ...
    #     "kda.graphql.payment.types.PaymentSource",
    ...
    @staticmethod
    def resolve_addresses(root: models.CustomUser, _info, **_kwargs):
        return root.addresses.annotate_default(root).all()

    # @staticmethod
    # def resolve_checkout(root: models.CustomUser, _info, **_kwargs):
    #     return get_user_checkout(root)[0]

    # @staticmethod
    # def resolve_gift_cards(root: models.CustomUser, info, **_kwargs):
    #     return root.gift_cards.all()

    @staticmethod
    def resolve_permissions(root: models.CustomUser, _info, **_kwargs):
        # deprecated, to remove in #5389
    ...
    @staticmethod
    def resolve_user_permissions(root: models.CustomUser, _info, **_kwargs):
        from .resolvers import resolve_permissions

        return resolve_permissions(root)

    @staticmethod
    def resolve_permission_groups(root: models.CustomUser, _info, **_kwargs):
        return root.groups.all()

    @staticmethod
    def resolve_editable_groups(root: models.CustomUser, _info, **_kwargs):
    ...
    def resolve_note(root: models.CustomUser, info):
        return root.note
    ...
    def resolve_events(root: models.CustomUser, info):
        return root.events.all()

    @staticmethod
    def resolve_orders(root: models.CustomUser, info, **_kwargs):
    ...
    @staticmethod
    def resolve_avatar(root: models.CustomUser, info, size=None, **_kwargs):
        if root.avatar:
    ...
    # @staticmethod
    # def resolve_stored_payment_sources(root: models.CustomUser, info):
    ...
    def resolve_private_meta(root: models.CustomUser, _info):
        return resolve_private_meta(root, _info)

    @staticmethod
    def resolve_meta(root: models.CustomUser, _info):
        return resolve_meta(root, _info)

    # @staticmethod
    # def resolve_wishlist(root: models.CustomUser, info, **_kwargs):
...
class StaffNotificationRecipient(CountableDjangoObjectType):
    ...
    class Meta:
        description = (
            "Represents a recipient of email notifications send by kda, "
