...
class PermissionGroupCreate(ModelMutation):
    ...
    def ensure_users_are_staff(
        ...
        if non_staff_users:
            # add error
            ids = [graphene.Node.to_global_id("CustomUser", pk) for pk in non_staff_users]
...
class PermissionGroupUpdate(PermissionGroupCreate):
    ...
    def ensure_requestor_can_manage_group(
        cls, requestor: "CustomUser", group: auth_models.Group
    ):
    ...
    def ensure_can_manage_users(
        ...
        if out_of_scope_users:
            # add error
            ids = [
                graphene.Node.to_global_id("CustomUser", user_instance.pk)
    ...
    def check_if_removing_user_last_group(
        cls, requestor: "CustomUser", errors: dict, cleaned_input: dict
    ):
        ...
        if requestor in remove_users and requestor.groups.count() == 1:
            ...
            params = {"users": [graphene.Node.to_global_id("CustomUser", requestor.pk)]}
