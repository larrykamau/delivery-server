...
from .models import CustomerEvent, CustomUser


def customer_account_created_event(*, user: CustomUser) -> Optional[CustomerEvent]:
...
def customer_password_reset_event(*, user: CustomUser) -> Optional[CustomerEvent]:
    return CustomerEvent.objects.create(user=user, type=CustomerEvents.PASSWORD_RESET)


def customer_password_changed_event(*, user: CustomUser) -> Optional[CustomerEvent]:
...
def customer_email_changed_event(
    *, user: CustomUser, parameters: dict
) -> Optional[CustomerEvent]:
    return CustomerEvent.objects.create(
...
def customer_placed_order_event(*, user: CustomUser, order: Order) -> Optional[CustomerEvent]:
...
def staff_user_deleted_a_customer_event(
    *, staff_user: CustomUser, deleted_count: int = 1
) -> CustomerEvent:
    return CustomerEvent.objects.create(
        user=staff_user,
        order=None,
...
def staff_user_assigned_email_to_a_customer_event(
    *, staff_user: CustomUser, new_email: str
) -> CustomerEvent:
    return CustomerEvent.objects.create(
        user=staff_user,
        order=None,
...
def staff_user_added_note_to_a_customer_event(
    *, staff_user: CustomUser, note: str
) -> CustomerEvent:
    return CustomerEvent.objects.create(
        user=staff_user,
        order=None,
...
def staff_user_assigned_name_to_a_customer_event(
    *, staff_user: CustomUser, new_name: str
) -> CustomerEvent:
    return CustomerEvent.objects.create(
        user=staff_user,
        order=None,
