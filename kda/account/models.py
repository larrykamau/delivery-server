...
from . import CustomerEvents, Vehicles
...
class CustomUser(AbstractBaseUser, PermissionsMixin, ModelWithMetadata):
    username = models.CharField(max_length=256, blank=True, unique=True)
    email = models.EmailField(unique=True)
    first_name = models.CharField(max_length=256, blank=True)
    last_name = models.CharField(max_length=256, blank=True)
    phone_number = PossiblePhoneNumberField(blank=True, default="254777777777")
...
class RiderInfo(models.Model):
    user = models.OneToOneField(
        CustomUser,
        related_name="rider_info",
        on_delete=models.CASCADE,
        blank=True,
        null=True,
    )
    name = models.CharField(max_length=256, blank=True)
    liscence_id = models.CharField(max_length=256, blank=True)
    is_active = models.BooleanField(default=True)

    class Meta:
        ordering = ("pk", "name",)

class VehicleInfo(ModelWithMetadata, models.Model):
    user = models.OneToOneField(
        CustomUser,
    ...
    type = models.CharField(
        max_length=50, choices=Vehicles.CHOICES, default=Vehicles.TWO
    )
    country_origin = models.CharField(max_length=256, blank=True)
    number_plate = models.CharField(max_length=256, blank=True)
    liscence_number = models.CharField(max_length=256, blank=True)
    inspected = models.BooleanField(default=False)

    class Meta:
        ordering = ("pk","liscence_number",)
