...
def order_created(order: "Order", user: "CustomUser", from_draft: bool = False):
...
@transaction.atomic
def cancel_order(order: "Order", user: "CustomUser"):
    """Cancel order.
...
def order_refunded(order: "Order", user: "CustomUser", amount: "Decimal",):
    events.payment_refunded_event(
        order=order, user=user, amount=amount, 
    )
    # get_plugins_manager().order_updated(order)


def order_voided(order: "Order", user: "CustomUser", ):
    events.payment_voided_event(order=order, user=user, )
    # get_plugins_manager().order_updated(order)
...
def order_captured(order: "Order", user: "CustomUser", amount: "Decimal"):
...
def fulfillment_tracking_updated(
    fulfillment: "Fulfillment", user: "CustomUser", tracking_number: str
):
...
def cancel_fulfillment(
    fulfillment: "Fulfillment", user: "CustomUser",):
    """Cancel fulfillment.
...
@transaction.atomic
def mark_order_as_paid(order: "Order", request_user: "CustomUser"):
    """Mark order as paid.
...
def _create_fulfillment_lines(
    fulfillment: Fulfillment, lines: List[Dict]
...
def create_fulfillments(
    ...
    Args:
        requester (CustomUser): Requester who trigger this action.
